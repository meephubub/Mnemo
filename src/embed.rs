...
//! Models" / section 80 Phase 4).
//!
//! Thin async wrapper over `mnemo-storage`'s embeddings repository:
//! embeds chunks that don't yet have a vector for the handle's
//! `Embedder`, and exposes counts/lookup/clear for that model. Swap
//! in a real local model (ONNX/Candle) by implementing
//! [`mnemo_embeddings::Embedder`] and passing it to
...
use mnemo_storage::repositories::{chunks, embeddings as embeddings_repo};
